...
### jazz_trio.js
Jazz piano trio over Autumn Leaves A-section changes, 120 BPM. **Uses evaluate mode** — strudel code built dynamically with data-driven interpolation. Cm7→F7→BbΔ7→EbΔ7→Am7b5→D7→Gm7→turnaround. **Data wiring:** Heat controls energy level — bass and ride always play (gain-scaled), layers gate in progressively: ghost snare + sparse comping (heat >0.2), cross-stick + full comping (>0.4), kick bombs + turnaround fill (>0.6). Melody responds to `price_move` (Oracle-style): ascending phrases when price rises, descending (original head) when price falls, silent when flat. Melody velocity scales with movement magnitude. Piano comping uses `chord().dict("ireal").voicing()` with 8 syncopated rhythms (sparse 4-bar at medium heat, full 8-bar at high). Walking bass (`gm_acoustic_bass`) has 16 bars (two choruses). Ride (`rd`) spang-a-lang, 8-bar hi-hat (simple 2&4 at low heat, full pattern at high). All layers use `.orbit()` for bus isolation. Spike events trigger crash cymbal. Caches output keyed on quantized heat + price direction + magnitude.
